...
<div class="container-fluid">
  <div class="row mt-3">
    <div class="col-4">
      <form action="{{ route('dayishowlist') }}" method="post" id="autosubmit">
        <select class="form-control form-control-lg" name="course_id" onchange="submitform();">
          @if($selvel == 0) 
          <option value="0" selected>所教课程</option>
          @endif
          @foreach($data as $val)
  ...
  <div class="">
      @if($selvel != 0)
      <button type="button" id="thecourque" class="btn btn-primary mt-2" data-toggle="modal" data-target="#staticBackdrop" value="{{$selvel}}" onclick="transmit()">提问题</button>
      @endif
   </div>
  <div class="row mt-3" id="ulfoldall" name="ulfoldall">
  @foreach($getque as $val2)
    <div class="col-6 mt-2 float-right card" >
      <div class="" id="hedding{{$val2->id}}">
          ...
          <button type="button" onclick="transmit2('{{$val2->id}}')" class="btn btn-outline-primary btn-sm float-right mt-2 mr-5" data-toggle="modal" data-target="#staticBackdrop2">回复</button>
      </div>
