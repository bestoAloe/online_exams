...
<body>
    ...
    <div class="page-container">
        <div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="javascript:;" onclick="course_add('添加课程','courseadd','800','500')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加课程</a></div>
        <table class="table table-border table-bordered table-bg">
            ...
            <tbody>
               @foreach ($data as $val)
                <tr class="text-c">
                    <td><input type="checkbox" value="{{$val->id}}" name="{{$val->id}}"></td>
                    <td>{{$val->id}}</td>
                    <td>{{$val->course_name}}</td>
                    <td>{{$val->rea_profession -> profession_name}}</td>
                    <td><img src="../{{$val-> cover_img}}" width="50"></td>
                    <td>{{$val->sort}}</td>
