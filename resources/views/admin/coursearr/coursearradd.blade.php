...
<![endif]-->
<title>添加课程安排</title>
<meta name="keywords" content="添加课程安排">
<meta name="classtime" content="添加课程安排">
</head>
<body>
<article class="page-container">
	<form class="form form-horizontal" id="form-admin-add">
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>选择课程：</label>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9"> <span class="select-box" style="width:150px;">
				<select class="select" name="manager_id" size="1">
					<option value="0"></option>
					@foreach($tmp as $val2)
					   <option value="{{$val2 -> id}}">{{$val2 -> username}}</option>
					@endforeach
				</select>
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-6 col-sm-7">
				<input type="text" class="input-text"   name="classtime" id="classtime">
			</div>
		</div>
...
$('select[name=protype_id]').change(function(){
  ...
  $.get('./arrgetprotyid',{protype_id:protype_id},function(prof){
    //jquery的循环操作
    let str = '';
...
$('select[name=profession_id]').change(function(){
  ...
  $.get('./arrgetfessid',{profession_id:profession_id},function(prof){
    //jquery的循环操作
    let str = '';
    $.each(prof,function(index,el){
